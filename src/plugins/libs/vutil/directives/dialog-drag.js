	...
	bind(el) {
		...
		const moveDropDom = (clientX, clientY) => {
			if (clientX && clientY) {
				...
				let pointChange = [movePoint[0] + clientChange[0], movePoint[1] + clientChange[1]];
				...
				if (
					...
					(clientRect.right > clientWidth - client_offset && clientChange[0] > 0)
				) {
				...
				if (
					...
					(clientRect.bottom > clientHeight - client_offset && clientChange[1] > 0)
				) {
		...
		dropDom.style.cssText += ';marign: 15vh auto 0;position: fixed;left: 0;right: 0';
		// 鼠标被按下
		proxyDropDom.onmousedown = function(e) {
			// 正则判断弹窗位置是否还原
			// eg: translate(0px, 44px)；translate(0, 44px)
			let transform = dropDom.style.transform;
			// ["translate(0px, 44px)", "0", "px", "44"]； null
			let res = /translate\((\d+)(px),\s?(\d+)\2\)/.exec(transform);
