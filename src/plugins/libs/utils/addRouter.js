 ...
 **/
import { router as routerInstance } from '@/plugins';
import Layout from '@libs/layout';
// import LoadingComponent from '@/packages/asyncloading';
import ErrorComponent from '@libs/packages/error-page/asyncerror';
import IframeComponent from '@libs/packages/web-iframe';
import upperFirst from 'lodash/upperFirst';
import camelCase from 'lodash/camelCase';
import { getUrlInfo, encodeString } from '@libs/utils/utils';
...
const formatterRouter = (router, props, route = [], menu = [], source = []) => {
	...
	if (Object.prototype.toString.call(router) === '[object Array]') {
		...
		if (isLink) {
			...
			if (queryIndex > -1) {
				path = path.replace('?', `?spaceApp=iframe&origin=${window.location.origin}&`);
			} else {
				path = path + `?spaceApp=iframe&origin=${window.location.origin}`;
			}
			tempRoute = {
				meta: { ...tempMenu.meta },
				name: key,
...
export const formatterRouters = (routers, props) => {
	...
	if (process.env.NODE_ENV === 'production' || !website.env.isMock) {
